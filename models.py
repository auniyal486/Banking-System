...
class User(db.Model, UserMixin):
    ...
    phone_number = db.Column(db.String(264), nullable=False)
...
class Transaction(db.Model):
    ...
    account_no = db.Column(db.Integer, db.ForeignKey('account.account_no'), nullable=False)
...
class Account(db.Model):
    account_no = db.Column(db.Integer, primary_key=True, nullable=False)
...
class CardDetails(db.Model):
    account_no = db.Column(db.Integer, db.ForeignKey('account.account_no'), nullable=False)
    atm_card_no = db.Column(db.Integer, primary_key= True, nullable = False)
